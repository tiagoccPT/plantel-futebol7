...
exact_replacements = [
    ...
    ("style: const TextStyle(color: _muted, fontSize: 11),\n                    overflow: TextOverflow.ellipsis,", "style: const TextStyle(color: _muted, fontSize: 10),\n                    overflow: TextOverflow.ellipsis,"),
    ("const SizedBox(height: 6),\n                  Wrap(\n                    spacing: 4,\n                    runSpacing: 4,", "const SizedBox(height: 3),\n                  Wrap(\n                    spacing: 3,\n                    runSpacing: 2,"),
...
for old, new in exact_replacements:
    if old not in text:
        raise SystemExit(f'Padrão obrigatório não encontrado: {old[:90]!r}')
...
if old_field not in text:
    raise SystemExit('Bloco principal do campo não encontrado.')
...
compact_field = """  Widget _compactField() {
...
"""
text = text.replace(marker, marker + compact_field, 1)
...
"""
text = text[:start] + bench_block + text[end:]

main_path.write_text(text, encoding='utf-8')

pubspec = pubspec_path.read_text(encoding='utf-8')
if 'version: 1.0.0+1' in pubspec:
    pubspec = pubspec.replace('version: 1.0.0+1', 'version: 1.0.1+2', 1)
elif 'version: 1.0.1+2' not in pubspec:
